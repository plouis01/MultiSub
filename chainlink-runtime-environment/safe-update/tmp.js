...
var TokenSchema = exports_external.object({
  ...
  type: exports_external.enum(["erc20", "aave", "morpho"])
});
...
var getPriceFromFeed = (runtime2, priceFeedAddress) => {
...
var decodeWithdrawalAmount = (runtime2, txData) => {
  try {
    const selector = txData.slice(0, 10);
    runtime2.log(`Transaction selector: ${selector}`);
    if (selector === "0x69328dec") {
      runtime2.log("Detected Aave withdraw function");
      const params = txData.slice(10);
      const asset = "0x" + params.slice(24, 64);
      const amount = BigInt("0x" + params.slice(64, 128));
      runtime2.log(`Aave withdrawal: ${amount} of token ${asset}`);
      return { amount, token: asset };
    }
    if (selector === "0xb460af94") {
      runtime2.log("Detected Morpho withdraw function");
      const params = txData.slice(10);
      const amount = BigInt("0x" + params.slice(0, 64));
      runtime2.log(`Morpho withdrawal: ${amount}`);
      return null;
    }
    if (selector === "0xba087652") {
      runtime2.log("Detected Morpho redeem function");
      const params = txData.slice(10);
      const shares = BigInt("0x" + params.slice(0, 64));
      runtime2.log(`Morpho redeem: ${shares} shares`);
      return null;
    }
    runtime2.log(`Unknown function selector: ${selector}`);
    return null;
  } catch (error) {
    runtime2.log(`Error decoding withdrawal: ${error}`);
    return null;
  }
};
var onProtocolExecuted = (runtime2, payload) => {
  runtime2.log("ProtocolExecuted event received");
  try {
    ...
    const subAccount = "0x" + log.topics[1].slice(-40);
    const target = "0x" + log.topics[2].slice(-40);
    runtime2.log(`Processing transaction for subAccount=${subAccount}, target=${target}`);
    if (!tx || !tx.data) {
      runtime2.log("No transaction data available");
      return "No transaction data";
    }
    runtime2.log(`Transaction data: ${tx.data}`);
    const txDataHex = tx.data;
    if (txDataHex.length < 10) {
      runtime2.log("Transaction data too short");
      return "Invalid transaction";
    }
    const dataOffset = 2 + 8 + 64 + 64;
    ...
    runtime2.log(`Extracted protocol calldata: ${protocolCalldata}`);
    const withdrawal = decodeWithdrawalAmount(runtime2, protocolCalldata);
    if (!withdrawal) {
      runtime2.log("Not a recognized withdrawal transaction");
      return "Not a withdrawal";
    }
    runtime2.log(`Detected withdrawal: ${withdrawal.amount} of token ${withdrawal.token}`);
    const tokenConfig = runtime2.config.tokens.find((t) => t.address.toLowerCase() === withdrawal.token.toLowerCase());
    if (!tokenConfig) {
      runtime2.log(`Token ${withdrawal.token} not in config, cannot calculate USD value`);
      return "Token not configured";
    }
    const { price, decimals: priceDecimals } = getPriceFromFeed(runtime2, tokenConfig.priceFeedAddress);
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName: runtime2.config.chainSelectorName,
      isTestnet: true
    ...
    runtime2.log(`Withdrawal value in USD: ${balanceChange} (${Number(balanceChange) / 1000000000000000000} USD)`);
    const callData = encodeFunctionData({
      abi: DeFiInteractorModule,
      functionName: "updateSubaccountAllowances",
      args: [subAccount, balanceChange]
    });
    runtime2.log(`Calling updateSubaccountAllowances for ${subAccount} with balanceChange=${balanceChange}`);
    ...
    if (txStatus !== TxStatus.SUCCESS) {
      runtime2.log(`Failed to update allowances: ${resp.errorMessage || txStatus}`);
      return `Failed: ${resp.errorMessage || txStatus}`;
    } else {
      const txHash = resp.txHash || new Uint8Array(32);
      runtime2.log(`Successfully updated allowances for ${subAccount}. TxHash: ${bytesToHex(txHash)}`);
